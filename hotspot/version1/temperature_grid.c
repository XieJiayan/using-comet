...
void dump_top_layer_temp_grid(grid_model_t *model, char *file, grid_model_vector_t *temp)
{
  ...
  int allGrids = model->n_layers * model->rows * model->cols;
  for (k = 0; k < EXTRA; k++)
    fprintf(fp, "%d\t%.2f\n", k,
            model->last_steady->cuboid[0][0][allGrids + k]);
    ...
    for (u = 0; u < model->layers[n].flp->n_units; u++)
    {
      ...
      if (model->map_mode == GRID_CENTER)
      {
        ...
        b[base + u] = (g->cuboid[n][ci1][cj1] + g->cuboid[n][ci2][cj1] +
                       g->cuboid[n][ci1][cj2] + g->cuboid[n][ci2][cj2]) /
                      4;
        continue;
      }
                        ...
                        grid_model_vector_t *temp)
{
  ...
  for (i = 0; i < nr; i++)
    for (j = 0; j < nc; j++)
      temp->cuboid[nl - 1][i][j] = model->config.ambient + sum[i][j] * model->layers[nl - 1].rz;
...
SuperMatrix build_steady_grid_matrix(grid_model_t *model)
{
  ...
  if (!(cooV = doubleMalloc(nnz)))
    fatal("Malloc fails for cooV[].\n");
  if (!(cooX = intMalloc(nnz)))
    fatal("Malloc fails for cooX[].\n");
  if (!(cooY = intMalloc(nnz)))
    fatal("Malloc fails for cooY[].\n");

  if (!(a = doubleMalloc(nnz)))
    fatal("Malloc fails for a[].\n");
  if (!(asub = intMalloc(nnz)))
    fatal("Malloc fails for asub[].\n");
  if (!(xa = intMalloc(n + 1)))
    fatal("Malloc fails for xa[].\n");
  ...
  for (l = 0; l < nl; l++)
  {
    ...
    for (i = 0; i < nr; i++)
      for (j = 0; j < nc; j++)
      {
        ...
        {
          if (j > 0)
            Rw = find_res_3D(l, i, j - 1, model, 1);
          else
            Rw = LARGENUM;
          if (j < nc - 1)
            Re = find_res_3D(l, i, j + 1, model, 1);
          else
            Re = LARGENUM;
          if (i > 0)
            Rn = find_res_3D(l, i - 1, j, model, 2);
          else
            Rn = LARGENUM;
          if (i < nr - 1)
            Rs = find_res_3D(l, i + 1, j, model, 2);
          else
            Rs = LARGENUM;
          if (l > 0)
            Ra = find_res_3D(l - 1, i, j, model, 3);
          else
            Ra = LARGENUM;
          if (l < nl - 1)
            Rb = find_res_3D(l, i, j, model, 3);
          else
            Rb = LARGENUM;
        }
        else
        ...
        { //top left corner
          ...
          cooY[curidx] = grididx + 1 + yoffset;
          cooV[curidx] = -1.0 / Re;
          curidx++;
          dia_val += 1.0 / Re;

          cooX[curidx] = grididx + xoffset;
          cooY[curidx] = grididx + nc + yoffset;
          cooV[curidx] = -1.0 / Rs;
          curidx++;
          dia_val += 1.0 / Rs;
          ...
          {
            cooX[curidx] = grididx + xoffset;
            cooY[curidx] = grididx + nr * nc + yoffset;
            cooV[curidx] = -1.0 / Rb;
            curidx++;
            dia_val += 1.0 / Rb;
          }
          ...
          if (l == spidx)
          {
            ...
            cooX[curidx] = grididx + xoffset;
            cooY[curidx] = nl * nr * nc + SP_N;
            cooV[curidx] = -1.0 / R_temp;
            curidx++;
            dia_val += 1.0 / R_temp;
            /* western boundary	- edge cell has half the rx		*/
            R_temp = lyr[l].rx / 2.0 + nr * model->pack.r_sp1_x;
            cooX[curidx] = grididx + xoffset;
            cooY[curidx] = nl * nr * nc + SP_W;
            cooV[curidx] = -1.0 / R_temp;
            curidx++;
            dia_val += 1.0 / R_temp;
          }
  ...
  cooX[curidx] = SINK_N + xoffset;
  cooY[curidx] = SINK_C_N + yoffset;
  cooV[curidx] = -1.0 / R_temp;
  curidx++;
  dia_val += 1.0 / R_temp;
  R_temp = pk->r_hs_per + pk->r_amb_per;
  dia_val += 1.0 / R_temp;
  cooX[curidx] = SINK_N + xoffset;
  cooY[curidx] = SINK_N + yoffset;
  cooV[curidx] = dia_val;
  curidx++;

  dia_val = 0;
  R_temp = pk->r_hs2_y + pk->r_hs;
  cooX[curidx] = SINK_S + xoffset;
  cooY[curidx] = SINK_C_S + yoffset;
  cooV[curidx] = -1.0 / R_temp;
  curidx++;
  dia_val += 1.0 / R_temp;
  R_temp = pk->r_hs_per + pk->r_amb_per;
  ...
  cooX[curidx] = SINK_W + xoffset;
  cooY[curidx] = SINK_C_W + yoffset;
  cooV[curidx] = -1.0 / R_temp;
  curidx++;
  dia_val += 1.0 / R_temp;
  R_temp = pk->r_hs_per + pk->r_amb_per;
  dia_val += 1.0 / R_temp;
  cooX[curidx] = SINK_W + xoffset;
  cooY[curidx] = SINK_W + yoffset;
  cooV[curidx] = dia_val;
  curidx++;

  dia_val = 0;
  R_temp = pk->r_hs2_x + pk->r_hs;
  cooX[curidx] = SINK_E + xoffset;
  cooY[curidx] = SINK_C_E + yoffset;
  cooV[curidx] = -1.0 / R_temp;
  curidx++;
  dia_val += 1.0 / R_temp;
  R_temp = pk->r_hs_per + pk->r_amb_per;
  ...
  curidx++;
  dia_val += 1.0 / R_temp;
  R_temp = pk->r_hs_c_per_y + pk->r_amb_c_per_y;
  dia_val += 1.0 / R_temp;
  ...
  curidx++;
  dia_val += 1.0 / R_temp;
  R_temp = pk->r_hs_c_per_x + pk->r_amb_c_per_x;
  dia_val += 1.0 / R_temp;
  ...
  if (model_secondary)
  {
    ...
    cooX[curidx] = PCB_N + xoffset;
    cooY[curidx] = PCB_C_N + yoffset;
    cooV[curidx] = -1.0 / R_temp;
    curidx++;
    dia_val += 1.0 / R_temp;
    R_temp = pk->r_amb_sec_per;
    dia_val += 1.0 / R_temp;
    cooX[curidx] = PCB_N + xoffset;
    cooY[curidx] = PCB_N + yoffset;
    cooV[curidx] = dia_val;
    curidx++;

    dia_val = 0;
    R_temp = pk->r_pcb2_y + pk->r_pcb;
    cooX[curidx] = PCB_S + xoffset;
    cooY[curidx] = PCB_C_S + yoffset;
    cooV[curidx] = -1.0 / R_temp;
    curidx++;
    dia_val += 1.0 / R_temp;
    R_temp = pk->r_amb_sec_per;
    ...
    cooX[curidx] = PCB_W + xoffset;
    cooY[curidx] = PCB_C_W + yoffset;
    cooV[curidx] = -1.0 / R_temp;
    curidx++;
    dia_val += 1.0 / R_temp;
    R_temp = pk->r_amb_sec_per;
    dia_val += 1.0 / R_temp;
    cooX[curidx] = PCB_W + xoffset;
    cooY[curidx] = PCB_W + yoffset;
    cooV[curidx] = dia_val;
    curidx++;

    dia_val = 0;
    R_temp = pk->r_pcb2_x + pk->r_pcb;
    cooX[curidx] = PCB_E + xoffset;
    cooY[curidx] = PCB_C_E + yoffset;
    cooV[curidx] = -1.0 / R_temp;
    curidx++;
    dia_val += 1.0 / R_temp;
    R_temp = pk->r_amb_sec_per;
  ...
  for (l = 0; l < nl; l++)
    for (i = 0; i < nr; i++)
      for (j = 0; j < nc; j++)
      {
        ...
        {
          (*rhs)[idx] = power->cuboid[l][i][j] + c->ambient / (model->config.r_convec_sec * (model->config.s_pcb * model->config.s_pcb) / (cw * ch));
        }
        else
        {
...
// Darong Huang added
void dumpA(char *what, SuperMatrix *A)
{
  NCformat *Astore;
  register int i, n;
  double *dp;
...
void direct_SLU(grid_model_t *model, grid_model_vector_t *power, grid_model_vector_t *temp)
{
  ...
  dumpB("B", &B);
  // Darong Huang modified
  if (!(perm_r = intMalloc(dim)))
    fatal("Malloc fails for perm_r[].\n");
  if (!(perm_c = intMalloc(dim)))
    fatal("Malloc fails for perm_c[].\n");
